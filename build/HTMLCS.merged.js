    ...
    testNullAltText: function (top) {
        ...
        for (var el = 0; el < elements.length; el++) {
            ...
            if (element.hasAttribute('alt') === false) {
                ...
                similarAlt = HTMLCS.analizer.img.isSimilarAlt(element.getAttribute('alt'), element.getAttribute('src'))
            }
...
_global.HTMLCS.analyzer.img = function () {
    ...
    self.processor = {
        isSimilarAt: function (altText, imageSource, beStrict) {
            var result = false;
    ...
    self.isSimilarTagsToAlt = function (tags, altText) {
        ...
        tags.forEach(function (tag) {
            ...
            alts.forEach(function (alt) {
                ...
                isSimilar = isSimilar || alt.toLowerCase() === tag.description.toLowerCase();
            });
        });
...
_global.HTMLCS.providers.gcv = function () {
    ...
    self.isSimilarAlt = async function (altText, imageSource, beStrict, compareFunc) {
        var isSimilar = true;
        try {
            var client = new vision.ImageAnnotatorClient();
            var labels = await client.labelDetection(imageSource);
            isSimilar = compareFunc(labels, altText)
        } catch (e) {
            console.error('HTMLCS.providers.gcv', e);
...
//# sourceMappingURL=HTMLCS.merged.js.map